...
Func_local *ast2llvmFunc(aA_fnDef f)
{
    ...
    emit_irs.push_back(L_Label(Temp_newlabel_named(name)));
    ...
    for (auto &decl : f->fnDecl->paramDecl->varDecls)
    {
        if (decl->kind == A_varDeclType::A_varDeclScalarKind)
        {
            ...
            if (decl->u.declScalar->type->type == A_dataType::A_nativeTypeKind)
            {
                ...
                emit_irs.push_back(
                    L_Store(AS_Operand_Temp(arg), arg_ptr));
    ...
    else if (s->kind == A_varDefKind)
    {
        if (s->u.varDef->kind == A_varDefScalarKind)
        {
            ...
            else if (varDefScalar->type->type == A_structTypeKind)
            {
                // 只能一个一个个赋值
                auto temp =
                    Temp_newtemp_struct_ptr(0, *varDefScalar->type->u.structType);
                localVarMap.emplace(*varDefScalar->id, temp);
                emit_irs.push_back(L_Alloca(AS_Operand_Temp(temp)));
            }
            else
        ...
        else if (s->u.varDef->kind == A_varDefArrayKind)
        {
            ...
            auto id = *varDefArray->id;
            auto len = varDefArray->len;
            auto vals = varDefArray->vals;

            if (varDefArray->type->type == A_nativeTypeKind)
            {
                Temp_temp *temp = Temp_newtemp_int_ptr(len);
                temp->varname = string(*(s->u.varDef->u.defArray->id));
                ...
                for (; i < vals.size(); i++)
                {
                    ...
                    emit_irs.push_back(L_Store(ast2llvmRightVal(vals[i]), new_ptr));
                }
                for (; i < len; i++)
                {
                    AS_operand *new_ptr = AS_Operand_Temp(Temp_newtemp_int_ptr(0));
                    ...
                    emit_irs.push_back(L_Store(AS_Operand_Const(0), new_ptr));
                }
            }
...
void ast2llvmStmtreturn(aA_returnStmt s)
{
    if (s->retVal != nullptr)
    {
        auto retVal = ast2llvmRightVal(s->retVal);
...
AS_operand *ast2llvmMember_2(aA_memberExpr a)
{
...
AS_operand *ast2llvmId_1(string id)
{
    if (localVarMap.find(id) != localVarMap.end())
    {
        ...
        {
            AS_operand *res = AS_Operand_Temp(Temp_newtemp_struct_ptr(-1, temp->u.TEMP->structname));
            AS_operand *index = AS_Operand_Const(0);
            gep(res, temp, index);
            return res;
        }
...
AS_operand *ast2llvmRightVal(aA_rightVal r)
{
    ...
    if (r->kind ==
        A_rightValType::A_arithExprValKind)
    {
...
LLVMIR::L_func *ast2llvmFuncBlock(Func_local *f)
{
    ...
    if (!blockIr.empty())
    {
        if (blockIr.size() == 1)
        {
            ...
            {
                blockIr.push_back(L_Ret(AS_Operand_Const(0)));
            }
            else if (f->ret.type == ReturnType::VOID_TYPE)
            {
                blockIr.push_back(L_Ret(nullptr));
            }
            else if (f->ret.type == ReturnType::STRUCT_TYPE)
            {
                blockIr.push_back(L_Ret(AS_Operand_Temp(Temp_newtemp_struct_ptr(0, f->ret.structname))));
            }
        }
        blocks.push_back(L_Block(blockIr));
    }
