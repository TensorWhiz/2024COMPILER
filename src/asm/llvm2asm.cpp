...
ASM::AS_reg *immReg(int imm, list<AS_stm *> &as_list, ASM::AS_type type)
{

    unsigned int unsignedImm = (unsigned int)imm;
    if (unsignedImm > UINT_16_MAX)
    {
        auto dst = new AS_reg(type, Temp_newtemp_int()->num);
        as_list.push_back(AS_Movz(new AS_reg(IMM, unsignedImm >> 16), dst, 16));
        as_list.push_back(AS_Movk(new AS_reg(IMM, unsignedImm & 0xffff), dst));
        return dst;
    }
    return new AS_reg(IMM, imm);
}
...
void new_frame(list<AS_stm *> &as_list, L_func &func)
{
    // Fixme: add here
    // TODO: 调整栈指针
    as_list.emplace_back(AS_Mov(immReg(stack_frame, as_list, Xn), new AS_reg(AS_type::Xn, XXn1)));
...
void llvm2asmCJmp(list<AS_stm *> &as_list, L_stm *cjmp_stm)
{
    ...
    if (cjump->dst->kind == OperandKind::ICONST)
    {
        if (cjump->dst->u.ICONST == 1)
            as_list.push_back(AS_B(new AS_label(cjump->true_label->name)));
        else
...
void llvm2asmGep(list<AS_stm *> &as_list, L_stm *gep_stm)
{
    ...
    AS_reg *size_reg = new AS_reg(AS_type::Wn, Temp_newtemp_int()->num);
    as_list.emplace_back(AS_Mov(immReg(size, as_list, Wn), size_reg));
    //  计算偏移量，并记录在index_reg
    AS_reg *index_reg;
    if (gep_tmp->index->kind == OperandKind::TEMP)
    {
        index_reg = new AS_reg(AS_type::Wn, gep_tmp->index->u.TEMP->num);
    }
    else if (gep_tmp->index->kind == OperandKind::ICONST)
    {
        index_reg = new AS_reg(AS_type::Wn, Temp_newtemp_int()->num);
        as_list.emplace_back(AS_Mov(immReg(gep_tmp->index->u.ICONST, as_list, Wn), index_reg));
        ;
    }

    // 通过乘法，得到offset
    int offset_reg_num = Temp_newtemp_int()->num;
    AS_reg *offset_reg_wn = new AS_reg(AS_type::Wn, offset_reg_num);
    AS_reg *offset_reg_xn = new AS_reg(AS_type::Xn, offset_reg_num);
    as_list.emplace_back(AS_Binop(AS_binopkind::MUL_, size_reg, index_reg, offset_reg_wn));
    ...
    as_list.emplace_back(AS_Binop(AS_binopkind::ADD_, base_reg, offset_reg_xn, new AS_reg(AS_type::Xn, gep_tmp->new_ptr->u.TEMP->num)));
}
    ...
    for (BLOCKPHI *block_phi : phi)
    {
        ...
        for (auto pair : phi->phis)
        {
            ...
            if ((*last_two_iterator)->type == AS_stmkind::B || (*last_two_iterator)->type == AS_stmkind::BL || (*last_two_iterator)->type == AS_stmkind::BCOND)
            {
                ...
                {
                    p->stms.insert(last_two_iterator, AS_Mov(new AS_reg(AS_type::IMM, operand->u.ICONST), new AS_reg(AS_type::Xn, phi->dst->u.TEMP->num)));
                }
                else if (operand->kind == OperandKind::TEMP)
                {
            ...
            else
            {
                if (operand->kind == OperandKind::ICONST)
                {
                    p->stms.insert(last_iterator, AS_Mov(new AS_reg(AS_type::IMM, operand->u.ICONST), new AS_reg(AS_type::Xn, phi->dst->u.TEMP->num)));
                }
                else if (operand->kind == OperandKind::TEMP)
                {
