...
ASM::AS_mov::AS_mov(AS_reg *_src, AS_reg *_dst)
    : src(_src), dst(_dst) {}
ASM::AS_movz::AS_movz(AS_reg *_src, AS_reg *_dst,int _shift) : src(_src), dst(_dst),shift(_shift)
{
...
AS_stm *ASM::AS_Movz(AS_reg *src, AS_reg *dst,int shift)
{
    auto p = new AS_stm();
    p->type = AS_stmkind::MOVZ;
    p->u.MOVZ = new AS_movz(src, dst,shift);
    return p;
}
