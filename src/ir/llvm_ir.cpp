...
void LLVMIR::L_stm::removeFromBlock()
{
    if (this->bb != nullptr)
        this->bb->instrs.remove(this);
}

L_block* LLVMIR::label2block(Temp_label* label,L_func* f){
...
// void L_block::remove_all_succs(L_func* f){
//     for (auto it = f->blocks.begin(); it != f->blocks.end(); ) {
//     auto bb = *it;
//     if (succs.find(bb->label) != succs.end()) {
