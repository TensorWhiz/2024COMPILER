...
AS_operand* get_stm_def_operand(L_stm *stm){
    switch (stm->type)
    {
    case L_StmKind::T_BINOP:
    {
    ...
    case L_StmKind::T_PHI:
    {
        if (stm->u.PHI->dst->kind == OperandKind::TEMP && stm->u.PHI->dst->u.TEMP->type == TempType::INT_TEMP)
        {
            return stm->u.PHI->dst;
        }
    }
    break;
    case L_StmKind::T_GEP:
    {
        if (stm->u.GEP->new_ptr->kind == OperandKind::TEMP && stm->u.GEP->new_ptr->u.TEMP->type == TempType::INT_PTR)
        {
            return stm->u.GEP->new_ptr;
        }
    }
    break;
    case L_StmKind::T_CJUMP:
...
std::list<AS_operand *> get_stm_use_operand(L_stm *stm)
{
    list<AS_operand *> AS_operand_list ;
    switch (stm->type)
    {
...
std::list<AS_operand **> get_def_int_operand(LLVMIR::L_stm *stm)
{
...
std::list<AS_operand **> get_use_int_operand(LLVMIR::L_stm *stm)
{
