...
{
    if(index->kind == A_numIndexKind) return AS_Operand_Const(index->u.num);
    else if(index->kind == A_idIndexKind) return ast2llvmId(*index->u.id);
    else{
        assert(0);
        return nullptr;
    }   
...
AS_operand *ast2llvmId(string id){
    if(localVarMap.find(id) != localVarMap.end()) return AS_Operand_Temp(localVarMap[id]);
    else if(globalVarMap.find(id) != globalVarMap.end())  return AS_Operand_Name(globalVarMap[id]);
    else{
        assert(0);
        return nullptr;
    }
    ...
    for (auto x : f->irs){
        switch (x->type){
            case L_StmKind::T_LABEL:{ 
                ...
                if (hasSuccs) lblock->succs.emplace(x->u.LABEL->label);
            ...
            case L_StmKind::T_RETURN:{
                if (notRet) lblock->instrs.push_back(x);
                notRet = false;
                break;
            }
            default:{
...
void ast2llvmStmtvarDecl(aA_varDeclStmt s){
    if(s->kind == A_varDeclKind){
        ...
        if(s->u.varDef->kind == A_varDefScalarKind){
            ...
            if(varDefArray->type->type == A_structTypeKind) assert(0);
            else{
...
void ast2llvmStmtif(aA_ifStmt s, Temp_label *con_label, Temp_label *bre_label){
    ...
    for(int i = 0; i < s->ifStmts.size(); i++){
        ...
        if(s->ifStmts[i]->kind == A_codeBlockStmtType::A_returnStmtKind) break;
    }
    ...
    for(int i = 0; i < s->elseStmts.size(); i++){
        ...
        if(s->elseStmts[i]->kind==A_codeBlockStmtType::A_returnStmtKind) break;
    }
    for(int i = 0; i < tempVar[tempVar.size() - 1].size(); i++){
        localVarMap.erase(tempVar[tempVar.size() - 1][i]);
    }
...
void ast2llvmStmtwhile(aA_whileStmt s){
    ...
    for(int i = 0; i < s->whileStmts.size(); i++){
        ...
        if(s->whileStmts[i]->kind == A_codeBlockStmtType::A_returnStmtKind) break;
    }
    for(int i = 0; i < tempVar[tempVar.size() - 1].size(); i++) localVarMap.erase(tempVar[tempVar.size() - 1][i]);
    tempVar.pop_back();
    jump(new_con_label);
...
void ast2llvmStmtreturn(aA_returnStmt s){
    if(s->retVal == nullptr) emit_irs.push_back(L_Ret(nullptr));
    else emit_irs.push_back(L_Ret(MyLoad(ast2llvmRightVal(s->retVal))));
}
