...
test_single() {
	...
	aarch64-linux-gnu-gcc -c $func_testcase_dir/$test_name.S -o output/asm/$test_name.o --static
    aarch64-linux-gnu-gcc output/asm/$test_name.o output/sylib.o -o output/asm/$test_name --static
    if [ $? != 0 ]; then
    ...
    if [ -f $func_testcase_dir/$test_name.in ]; then
        qemu-aarch64 ./output/asm/$test_name < $func_testcase_dir/$test_name.in > output/asm/$test_name.out
    else
        qemu-aarch64 ./output/asm/$test_name > output/asm/$test_name.out
    fi
    echo -e $? >> ./output/asm/$test_name.out
    diff -Bb ./output/asm/$test_name.out $func_testcase_dir/$test_name.out > /dev/null 2>/dev/null
    if [ $? == 0 ]; then
        echo pass
